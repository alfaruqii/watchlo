...
export const Banner = ({ item }: BannerProps) => {
  ...
  return (
    <div className="relative flex w-full overflow-hidden" key={item.id}>
      <div className={`absolute transition-all duration-600 ease-in-out inset-0 z-0 ${isImageLoading ? "blur-3xl" : "blur-0"}`}>
        <Image
          unoptimized
          alt={determineAlt()}
          src={getImageUrl()}
      ...
      <div className="relative inset-0 z-10 flex w-screen flex-col justify-around gap-4 bg-gradient-to-t from-black/80 via-black/50 to-transparent p-20 text-white sm:gap-5 md:gap-6 lg:p-32">
      </div>
    </div>
  );
};
